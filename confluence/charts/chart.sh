...
cat > /tmp/chart1 <<DOCHERE
{"id":"<PAGE_ID>",
"type":"page",
"title":"Projects vs Spent chart",
"space":{"key":"<SPACE_KEY>"},
"body":
{"storage":
{"value":
...
<ac:parameter ac:name=\"title\">Week $Week</ac:parameter>
...
while read -r Project; do
    ...
    for f in ${arr[@]}; do
        ...
        AND YEARWEEK(spent_on) = YEARWEEK(NOW() - INTERVAL 1 WEEK);")
...
\"representation\":\"storage\"}},\n\"version\":{\"number\":$Version}}" > /tmp/chart2

cat /tmp/{chart1,chart2} > /tmp/chart.json
./api.sh post chart.json <PAGE_ID>
